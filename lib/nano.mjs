...
import ChangesReader from './changesreader.mjs'
import CookieJar from './cookie.mjs'
import MultiPartFactory from './multipart.mjs'
...
export default function Nano(cfg) {
  ...
  if (cfg.parsedURL.username && cfg.parsedURL.password) {
    cfg.headers.Authorization = 'Basic ' + Buffer.from(`${decodeURIComponent(cfg.parsedURL.username)}:${decodeURIComponent(cfg.parsedURL.password)}`).toString('base64')
  }
  ...
  async function relax(relaxOpts) {
    ...
    const opts = { ...cfg, ...(relaxOpts || {}), headers: { ...cfg.headers, ...(relaxOpts?.headers || {}) } }
    ...
    if (typeof opts.qs === 'object') {
      for (const [key, value] of Object.entries(opts.qs)) {
        // add each k/v to the URL's searchParams, taking care to JSON.stringify certain items
    ...
    if (opts.method && opts.method.toLowerCase() === 'head') {
      ...
      // for streamed output
      output = Readable.fromWeb(response.body)
    } else if (contentType.startsWith(MIME_JSON) && !opts.dontParse) {
      // json is json
  ...
  function docModule(dbName) {
    ...
    async function destroyAtt(docName, attName, qs) {
      if (missing(docName, attName)) {
        return Promise.reject(invalidParametersError)
      }
