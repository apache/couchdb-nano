...
module.exports = exports = function dbScope (cfg) {
  ...
  if (cfg.agentOptions && typeof cfg.agentOptions === 'object') {
  ...
  const responseHandler = async function (response, req, opts, resolve, reject, callback) {
    ...
    if (!response.status) {
      ...
      if (callback) {
        const returnError = new Error(`error happened in your connection. Reason: ${response.statusText}`)
    ...
    if (statusCode >= 200 && statusCode < 400) {
      ...
      } else if (ct && (ct.startsWith('text/') || ct.startsWith('multipart/related'))) {
  ...
  function relax (opts, callback) {
    ...
    const req = Object.assign({
      ...
      headers: Object.assign(headers, cfg.headers ? cfg.headers : {})
    })
    ...
    if (opts.form) {
      ...
      req.body = new URLSearchParams(opts.form)
    }
    ...
    req.params = req.qs
    req.serializedParams = new URLSearchParams(req.params)
    req.data = req.body
    delete req.body
    req.maxRedirects = 0
    if (opts.stream) {
    ...
    Object.assign(req.headers, cfg.headers)

    const fetchOptions = {
      method: req.method,
      headers: req.headers,
      credentials: 'include',
      body: req.method !== 'get' && req.data ? req.data : undefined,
      redirect: 'error',
      keepalive: true,
      signal: req.signal
    }
    ...
    const queryString = req.serializedParams.toString()
    ...
    const cookie = cookieJar.getCookieStringSync(req.url)
    if (cookie) {
    ...
    if (opts.stream) {
      ...
      fetch(req.url, fetchOptions)
        .then(async (response) => {
          ...
          if (response.status > 300) {
            streamResponseHandler(response, req, outStream)
          } else {
      ...
      if (typeof callback === 'function') {
        fetch(req.url, fetchOptions).then((response) => {
